...
INSERT INTO snowflake_schema.dim_customers (first_name, last_name, age, email, country, postal_code)
SELECT DISTINCT 
    customer_first_name,
    customer_last_name,
    customer_age,
...
INSERT INTO snowflake_schema.dim_pets (customer_id, pet_type, pet_name, pet_breed)
SELECT DISTINCT 
    c.customer_id,
    m.customer_pet_type,
    m.customer_pet_name,
...
FROM mock_data m
JOIN snowflake_schema.dim_customers c ON 
...
INSERT INTO snowflake_schema.dim_sellers (first_name, last_name, email, country, postal_code)
SELECT DISTINCT 
    seller_first_name,
    seller_last_name,
    seller_email,
...
INSERT INTO snowflake_schema.dim_products (name, category, price, weight, color, size, brand, material, description, rating, reviews, release_date, expiry_date, pet_category)
SELECT DISTINCT 
    product_name,
    product_category,
    product_price,
...
INSERT INTO snowflake_schema.dim_stores (name, location, city, state, country, phone, email)
SELECT 
    store_name,
    store_location,
    store_city,
...
INSERT INTO snowflake_schema.dim_suppliers (name, contact, email, phone, address, city, country)
SELECT 
    supplier_name,
    supplier_contact,
    supplier_email,
...
INSERT INTO snowflake_schema.fact_sales (customer_id, seller_id, product_id, store_id, supplier_id, sale_date, quantity, total_price, unit_price)
SELECT 
    c.customer_id,
    s.seller_id,
    p.product_id,
...
JOIN snowflake_schema.dim_customers c ON 
    ...
    m.customer_age = c.age AND 
    m.customer_country = c.country
JOIN snowflake_schema.dim_sellers s ON 
...
JOIN snowflake_schema.dim_products p ON 
